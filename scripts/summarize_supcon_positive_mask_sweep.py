...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize SupCon positive-mask sweep results.")
    parser.add_argument(
        "--output-root",
        default="outputs_supcon_positive_mask_sweep",
        help="Root directory containing sweep outputs.",
    )
    parser.add_argument(
        "--output-csv",
        default="supcon_positive_mask_sweep_summary.csv",
...
def bool_sort_value(value: Any) -> int:
...
def sort_key(row: Mapping[str, Any]) -> tuple[int, str, float, float, int]:
    ...
    return (
        bool_sort_value(row.get("include_same_domain_same_class")),
        str(row.get("supcon_positive_mode", "")),
...
def print_table(rows: List[Mapping[str, Any]]) -> None:
    headers = ["inc_same", "mode", "lambda", "temp", "seed", "acc", "f1", "pos_frac", "pos_mean"]
    table_rows = [
        [
            str(row.get("include_same_domain_same_class", "")),
            str(row.get("supcon_positive_mode", "")),
