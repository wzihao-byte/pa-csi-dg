...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize SupCon lambda warmup sweep results.")
    parser.add_argument(
        "--output-root",
        default="outputs_supcon_warmup_sweep",
        help="Root directory containing sweep outputs.",
    )
    parser.add_argument(
        "--output-csv",
        default="supcon_warmup_sweep_summary.csv",
...
def sort_key(row: Mapping[str, Any]) -> tuple[float, int, int]:
    ...
    return (
        ...
        as_int(row.get("lambda_supcon_warmup_epochs"), default=10**9),
        as_int(row.get("seed")),
    )
...
def print_table(rows: List[Mapping[str, Any]]) -> None:
    headers = ["lambda", "warmup", "seed", "acc", "f1", "target", "effective", "supcon/ce"]
    table_rows = [
        [
            format_value(row.get("lambda_supcon"), precision=3),
            str(row.get("lambda_supcon_warmup_epochs", "")),
            str(row.get("seed", "")),
            format_value(row.get("test_accuracy")),
            format_value(row.get("test_f1_macro")),
            format_value(row.get("lambda_supcon_target"), precision=3),
            format_value(row.get("lambda_supcon_effective"), precision=3),
